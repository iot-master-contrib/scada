...
export class RequestService {
  ...
  request(method: string, uri: string, options: any): Observable<any> {
    let hideErrorMsg = false;
    if (options && options.params && options.params.hideErrorMsg) {
      hideErrorMsg = options.params.hideErrorMsg;
      delete options.params.hideErrorMsg
    }
    // delete options.hideErrorMsg;
